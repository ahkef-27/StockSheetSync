...
このツールは、Google スプレッドシート上で
米国主要3銘柄（Apple・Meta・Google）の株価データを自動取得・可視化するシステムです。

トリガーを設定することで、
**1日単位の推移（createDailyChart）**と、
**過去7日間のまとめ（createSummaryCharts）**を自動で作成します。

### ⚙️ トリガー実行後の処理（1日ごとの自動処理）

トリガーを 30分ごと に設定すると、米国市場の取引時間（日本時間22時〜翌5時） の間に以下の処理が自動で実行されます。


### 🕒 日次処理（30分ごとトリガー実行）

トリガーを30分ごとに設定すると、
米国市場の取引時間（日本時間22時〜翌5時）の間に以下の処理が自動で行われます。

1. 株価データの取得と記録

Apple（AAPL）、Meta（META）、Google（GOOGL）の株価を取得し、
取得時刻とともに 「本日株価」 シートへ追記します。

→ 1日の値動きが30分単位で記録されていきます。

2. 一時シート tempData の生成と削除

各取得処理時に、データ格納用の一時シート tempData が自動生成されます。

全銘柄の処理が完了すると、自動的に削除されます。

3. 日次グラフの作成（createDailyChart）

「本日株価」シートのデータから、Apple・Meta・Google それぞれの折れ線グラフを作成。

→ 1日の値動きを時系列で視覚的に確認できます。

4. 書式調整

取得時刻（A列）は「hh:mm」形式に統一。

行の高さ・フォントサイズを自動調整し、可読性を向上。

これらによって、1日分の動きがひと目で把握できるようになっており、基本的には以下のような画面になります：
...
各銘柄（AAPL, META, GOOGL）の過去7日間の株価データを GOOGLEFINANCE 関数で取得。

取得データは一時的に tempData シートに展開されます。

2. グラフ用シートの作成（株価グラフ）

既存の週次グラフがある場合は上書きし、
「株価グラフ」 シートに3銘柄の7日間の株価推移を折れ線グラフで描画します。

値上がり・値下がり率に応じてセル背景色を自動で変更。

3. 一時シート削除

データ取得に使用した tempData シートは処理後に削除されます。

📄 処理後に残るシート

シート名　　　　　内容

本日株価　　　　　30分ごとの株価データが時系列で記録される

株価グラフ　　　　当日分の株価推移を折れ線グラフで可視化

下に示す週次グラフの部分で詳しく説明しています。
