...
トリガーを30分ごとに実行するように設定すると、スクリプトは米国市場の取引時間（日本時間22時〜翌5時）の間だけ自動的に動作します。
そのたびに以下の処理が行われます。

株価データの取得と記録
　スクリプトが Apple（AAPL）、Meta（META）、Google（GOOGL）の現在の株価を取得します。
　取得時刻とともに、「本日株価」シートに1行ずつ追記されていきます。
　→ これにより、1日分の株価変動が30分ごとに蓄積されます。

一時シート（tempData）の生成と削除
　株価取得中に、一時的なデータ処理用シート tempData が自動的に作成されます。
　このシート内で各銘柄のデータを一時的に記録しますが、処理がすべて完了すると 自動的に削除 されます。
　（そのため、ユーザーが見るのは基本的に「本日株価」と「株価グラフ」のみです）

日次グラフ（createDailyChart）の作成
　「本日株価」シート内のデータをもとに、
　- Apple
　- Meta
　- Google
　の3つの折れ線グラフが自動生成されます。
　それぞれの株価推移が時刻ごとに表示され、1日の値動きがひと目で確認できます。

表示形式と装飾
　時刻（A列）は「hh:mm」形式で表示され、
　各銘柄の価格軸は自動でスケーリング（最大・最小に余白を設定）されます。
　見やすさを意識した自動フォーマットも実行されます。
