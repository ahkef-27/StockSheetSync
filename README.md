...
株価グラフ	createDailyChart による当日の株価推移グラフと、createSummaryCharts による過去7日間の週次グラフ を可視化するシート。
...
これは Google Apps Script の deleteSheet() が重く、
22:00〜22:29 のリセット処理でタイムアウトが発生しやすいため、
安定稼働のために軽量なリセット方式を採用しています。
...
週次処理で使用する temp シートとは異なり、
「本日株価」シートは削除せず、毎日クリアして再利用しています。

これにより、スプレッドシートのロックやタイムアウトを防ぎ、
夜間の株価取得が安定して動作するようになっています。
...
処理内容ごとに複数の.gsファイルへ分割し、
役割が明確で拡張しやすい構造になるよう設計しています。
