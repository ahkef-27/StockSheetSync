...
このツールは、Google スプレッドシート上で株価データを自動的に取得・記録・可視化するスクリプトです。
トリガーを設定するだけで、Apple、Meta、Google の株価を30分ごとに取得し、グラフとして表示します。
...
Apple（AAPL）、Meta（META）、Google（GOOGL）の現在の株価を取得します。

取得時刻とともに 「本日株価」シート に追記され、1日の株価変動が30分単位で記録されていきます。

### 2️⃣ 一時シート（tempData）の生成と削除

スクリプト実行時に、データ取得・処理のための一時シート tempData が自動で作成されます。

各銘柄のデータを順に記録し、すべての処理が完了すると 自動的に削除 されます。
→ 処理後、スプレッドシート上には残りません。

### 3️⃣ 日次グラフ（createDailyChart）の作成

「本日株価」シートのデータをもとに、
Apple、Meta、Google の 3銘柄の折れ線グラフ を自動生成します。

グラフには当日の値動きが時系列で表示され、
→ 1日の変化を直感的に把握できます。
...
行の高さやフォントサイズも自動で調整され、見やすいレイアウトに整えられます。
